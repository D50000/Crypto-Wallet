...
function toggleCheckbox(coinId, coinSymbol, e) {
  if (document.getElementById(`checkbox_${coinId}`).checked == true) {
    // console.log(e);
    document
      .getElementById(`cryptoVolume_${coinId}`)
      .classList.remove("cryptoVolume");
  } else {
    document.getElementById(`cryptoVolume_${coinId}`).value = "";
    document
      .getElementById(`cryptoVolume_${coinId}`)
      .classList.add("cryptoVolume");
  }
}
...
function createSearchList(objectData) {
  Object.entries(objectData).forEach((entry) => {
    ...
    inputbox_node.setAttribute(
      "placeholder",
      `Input ${entry[1].symbol} volume ...`
    );
    ...
    document
      .getElementById(`checkbox_${entry[0]}`)
...
function deleteCache() {
  ...
  for (let i = 0; i < myCryptoWallet.length; i++) {
    ...
    document
      .getElementById(`cryptoVolume_${myCryptoWallet[i].id}`)
      .classList.add("cryptoVolume");
...
function getDrawData() {
  const detailCryptoList = fetch("https://api.coinmarketcap.com/v2/ticker/");
  detailCryptoList
    .then((result) => result.json())
    .then((result) => {
      drawData = [];
      ...
      for (let i = 0; i < myCryptoWallet.length; i++) {
        ...
        drawData.push(temp);
        temp2.value =
          myCryptoWallet[i].volume * result.data[key].quotes.USD.price;
